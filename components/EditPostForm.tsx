...
const EditPostForm = ({ post }: { post: Tpost }) => {
    ...
    useEffect(() => {
        ...
        initValues();
    }, [post.title, post.desc, post.content, post.links, post.imageUrl, post.publicId, post.catName])
        ...
        <>
            <div className=''>
                ...
                <form action="" className='flex flex-col gap-4' onSubmit={handleSubmit}>
                    ...
                    <input onChange={(e) => setTitle(e.target.value)} value={title} type="text" placeholder='Enter Title' className=' dark:bg-gray-800 cursor-pointer w-full placeholder:text-base placeholder:font-medium border-2 border-gray-400 p-2 rounded-md hover:outline-none outline-none' />
                    <textarea onChange={(e) => setDesc(e.target.value)} value={desc} rows={2} placeholder='Enter Short Description' className='dark:bg-gray-800 cursor-pointer resize-none w-full placeholder:text-base placeholder:font-medium border-2 border-gray-400 p-2 rounded-md hover:outline-none outline-none' />
                    <textarea onChange={(e) => setContent(e.target.value)} value={content} rows={8} placeholder='Enter Your Content' className='dark:bg-gray-800 cursor-pointer resize-none w-full placeholder:text-base placeholder:font-medium border-2 border-gray-400 p-2 rounded-md hover:outline-none outline-none' />
                    ...
                    <ul>
                        {
                            links && links.map((link, i) => (
                                <li className='flex items-center gap-2 mb-1 -mt-1  ' key={i}>
                                    ...
                                    <Link href={link} aria-label='Link' className=' text-gray-900 dark:text-gray-300 inline-block whitespace-nowrap max-w-full overflow-hidden text-ellipsis   font-semibold  '>{link}</Link>
                   ...
                   text-white dark:text-black bg-gray-800 dark:bg-gray-200 dark:hover:bg-gray-400  '><MdOutlineLibraryAdd size={20} />Add</button>
                    ...
                    {/* //Image Upload */}
                    <CldUploadButton uploadPreset={process.env.NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET} onUpload={handleImageUpload} className={` relative w-full h-60 sm:h-80 justify-center items-center flex bg-slate-200  dark:bg-gray-800  border-2  border-gray-400  rounded-md ${imageUrl && " pointer-events-none"}`}>
                        <div >
                            <BiImageAdd size={35} />
                        </div>
                    ...
                    <select onChange={(e) => setSelectedCategory(e.target.value)} value={selectedCategory} name='Select' className='dark:bg-gray-800 text-gray-600 dark:text-gray-400 font-medium cursor-pointer w-full relative border-2 border-gray-400 p-2 rounded-md hover:outline-none outline-none pr-4 appearance-none overflow-hidden mr-12 ' >
                    ...
                    <button className=' rounded-md font-medium p-2 text-lg text-white dark:text-black bg-gray-800 dark:bg-gray-200 dark:hover:bg-gray-400 hover:scale-[.99] 
                      transition-all duration-500  '>Update Post</button>
